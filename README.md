...
- 풀이·연습 화면 왼쪽에 **서버 목록 레일**이 있다. 문제가 제시하는 서버(단일 VM은
  localhost, 양단 문제는 host-a/host-b)가 상태 표시등과 함께 나열되고, 클릭하면 그
  서버의 터미널 세션이 열린다.
- **⧉ 세션 복제**: 서버당 셸을 최대 2개 열 수 있다 (게스트의 ttyS0/ttyS1). ping을
  보내면서 tcpdump로 관찰하는 식의 두-창 작업을 어느 문제에서든 할 수 있다.
  세션을 닫아도 셸은 살아 있어(창만 숨김) 다시 열면 실행 중이던 tcpdump가 그대로다.
- 터미널은 실제 콘솔처럼 보이게 했다: 창 크롬(타이틀바·ttyS 배지), Windows Terminal
  'Campbell' 팔레트, 컬러 프롬프트 `root@host:~#` (busybox ash의 `\[ \] \e` PS1
  이스케이프 지원을 실VM으로 검증해 적용).
