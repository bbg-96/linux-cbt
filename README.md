...
같은 엔진·같은 게스트 이미지 위에서 사이트마다 다른 문제 세트를 배포한다.

**① 기초 문법 트랙 — 운영 사이트, 8 카테고리 30문제** (`src/problems/all.ts`)
...
**② 클라우드 실무 트랙 — 스테이징 사이트, 6 카테고리 31문제** (`src/problems/staging.ts`,
`data/cloud/*.ts`) — 실제 장애 대응 순서를 따라간다:
초동 점검(증적 수집·프로세스·포트) → 서비스와 로그(OpenRC·Nginx·헬스체크) →
네트워크와 DNS(경로 선택·이름 해석·방화벽 순서·양단 검증) → 스토리지와 백업(inode·마운트·
rsync·체크섬) → 보안과 운영 증적(Agent 3축 증적·권한·바인딩) → 자동화(cron·bash -n·
보존정책·RPM DB 재구축).

`CategoryId`는 두 트랙의 카테고리를 모두 담고, 문제가 없는 카테고리는 화면에서 자동으로
숨겨진다(`network`만 두 트랙이 공유). 그래서 각 사이트에는 자기 트랙의 카테고리만 보인다.
